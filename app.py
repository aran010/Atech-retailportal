...
def main():
    ...
    with st.form("document_form", clear_on_submit=False):
        ...
        with st.expander("Registered Pharmacist Details"):
            ...
            with col_d:
                ...
                rp_reg_valid_upto = st.date_input(
                    "Reg. Valid Upto", value=None,
                    min_value=MIN_DATE, max_value=MAX_DATE,
                    key="rp_reg_valid_upto",
                )

            # Full-width row below columns
            col_e, col_f = st.columns(2)
            with col_e:
                rp_qualification = st.text_input(
                    "Qualification",
                )
                rp_prev_firm_name = st.text_input(
                    "Previous Firm Name",
                )
            with col_f:
                rp_college = st.text_input(
                    "College / Institute",
                )
                rp_prev_firm_address = st.text_input(
                    "Previous Firm Address",
                )

            c_res, _ = st.columns(2)
            with c_res:
                rp_resign_date = st.date_input(
                    "Resignation Date (from previous firm)",
                    value=None,
                    min_value=MIN_DATE, max_value=MAX_DATE,
                    key="rp_resign_date",
                )
    ...
    if submitted:
        ...
        context = {
            ...
            "rp_salary_words": amount_to_words(rp_salary),
            "rp_joining_date": fmt_date(rp_joining_date),
            "rp_reg_number": rp_reg_number.strip(),
            "rp_reg_date": fmt_date(rp_reg_date),
            "rp_reg_valid_upto": fmt_date(rp_reg_valid_upto),
            "rp_pharmacy_council": PHARMACY_COUNCIL,
            "rp_qualification": rp_qualification.strip(),
            "rp_college": rp_college.strip(),
            "rp_phone": rp_phone.strip(),
            "rp_prev_firm_name": rp_prev_firm_name.strip(),
            "rp_prev_firm_address": rp_prev_firm_address.strip(),
            "rp_resign_date": fmt_date(rp_resign_date),
            # Rent Agreement
